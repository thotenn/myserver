...
type AuthSessionConfig struct {
	...
	// once per dashboard, which means sessions do not survive a restart.
...
// AuthState is the atomically published authentication policy of ONE
// dashboard. Handlers and middleware must read it per request via
// Dashboard.Auth() so that edits to auth.yaml take effect without a restart.
...
// The session cookie's NAME is not a property of the policy alone: dashboards
// share a hostname, so the default carries the slug. It is Dashboard.CookieName.
...
// The policy itself lives on a Dashboard (see dashboard.go): Auth(),
// ReloadAuth() and the session-signing key are methods there, not package
// functions. Two dashboards in one process have two allowlists and two signing
// keys, and a package-level accessor is exactly the thing that cannot express
// that.
...
// UsesGeneratedSecret reports whether the session key is the per-dashboard
...
func readAuthFile(dir string) authRead {
